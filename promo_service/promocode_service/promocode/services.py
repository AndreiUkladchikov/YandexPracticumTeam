...
class PromocodeService:
    ...
    def _is_promocode_valid(self):
        ...
        logger.debug("Validate promocode %s: %s", self, str(self.__dict__))
    ...
    def _is_personal_promocode(self, user_id: uuid.UUID):
        ...
        logger.debug("Check personal promocode %s: %s", self, str(self.__dict__))
        if (
    ...
    def _is_promocode_spoiled(self, time_zone: pytz.utc):
        ...
        logger.debug("Checking the promocode expiration date %s: %s", self, str(self.__dict__))
    ...
    def _times_of_using_promocode(self) -> int:
        ...
        logger.debug(
            "Getting the count of promocode activations %s: %s", self, str(self.__dict__)
        )
    ...
    def _get_max_number_of_activations(self) -> int:
        ...
        logger.debug("Getting the maximum activation count %s: %s", self, str(self.__dict__))
    ...
    def _if_max_number_of_activations_exceed(self):
        ...
        logger.debug(
            "Checking the promocode for exceeding the maximum count of activations %s: %s",
            self,
            str(self.__dict__)
        )
    ...
    def _if_promocode_already_activated_by_current_user(self, user_id: uuid.UUID):
        """Checking the Promocode already activated by user."""
    ...
    def _add_to_history(self, user_id: uuid.UUID):
        ...
        logger.debug("Add promocode activate info to history %s: %s", self, str(self.__dict__))
        p = PromocodeUserHistory(promocode_id=self.promocode, user_id=user_id)
        p.save()

    def verify(self, user_id: uuid.UUID):
        """Verify promocode."""
        logger.debug("Verify promocode %s: %s", self, str(self.__dict__))
    ...
    def apply(self, user_id: uuid.UUID):
        """Apply promocode."""
        logger.debug("Apply promocode %s: %s", self, str(self.__dict__))
        self.verify(user_id)
...
def _get_promocode(promocode_value: str) -> Promocode | Exception:
    ...
    logger.debug("Get promocode %s", promocode_value)
...
def get_user_history(user_id: uuid.UUID):
    ...
    logger.debug("Get history for user %s", user_id)
